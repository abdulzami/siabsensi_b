const updateMahasiswa = (id, user) => {
    axios.put('https://siabsensi.herokuapp.com/api/mahasiswa/' + id, user, {
        headers: {
...
container.addEventListener('click', (e) => {
    ...
    if (updtButtonIsPressed) {
        let request = (req_name, req_gender, req_address, req_no_hp, req_majors) => {
            ...
            let mahasiswas = { name, jenis_kelamin, alamat, no_hp, jurusan }

            return mahasiswas;
        }

        let mahasiswa_edited = request("name", "gender", "address", "no_hp", "majors")

        updateMahasiswa(id, mahasiswa_edited);
