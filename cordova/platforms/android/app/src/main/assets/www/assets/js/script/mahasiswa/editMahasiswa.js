let dosen_data_edit = (data) => {
    return `
    <p class="h3 mb-4">Edit Student Data</p>
...
const fetchDosenEdit = () => {
    axios.get('https://siabsensi.herokuapp.com/api/mahasiswa/' + id, {
        headers: {
        ...
        .catch(error => {
            ...
            if (error.response) {
                let failed = `<p class="h3 mb-4">Edit Student Data</p>
                <span class="badge bg-${error.response.data.message[1]}">${error.response.data.message[0]}</span>`
                container.innerHTML = failed;
                console.error(error)

                if (error.response.status == 500) {
                    let failed = `<p class="h3 mb-4">Edit Student Data</p><span class="badge bg-info">
